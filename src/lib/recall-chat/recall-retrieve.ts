...
export function buildRecallAgentSystemPrompt(opts: {
  ...
  return [
    ...
    "You have the same tools as the wenlo MCP server, including search_library, grep_library, and create_library_item.",
    ...
    "Creating content (requires EDITOR access):",
    "- create_library_item kind=note: plain-text document (legacy name: create_note).",
    "- kind=page: BlockNote page (optional plain-text content → paragraphs).",
    "- kind=whiteboard | deck | database | flowchart: blank native item (database may use databaseTemplate: tasks, contacts, roadmap).",
    "- Cannot upload files or delete items. append_to_note / edit_note only work on notes, not pages or native docs.",
